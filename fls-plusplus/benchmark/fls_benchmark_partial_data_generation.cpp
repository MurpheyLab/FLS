...
int main(int argc, char** argv) {
    ...
    std::string pass_axis;
    float pass_min, pass_max;
    int num_pts;
    ...
    if(argc < 6) {
        ...
        cloud_file = argv[2];
        pass_axis = argv[3];
        pass_min = std::atof(argv[4]);
        pass_max = std::atof(argv[5]);
        num_pts = std::atoi(argv[6]);
    }
    ...
    // Cut the source cloud
    pcl::PassThrough<fls::Point> pass_filter;
    pass_filter.setInputCloud(src_cloud);
    pass_filter.setFilterFieldName(pass_axis);
    pass_filter.setFilterLimits(pass_min, pass_max);
    pass_filter.filter(*src_cloud);
